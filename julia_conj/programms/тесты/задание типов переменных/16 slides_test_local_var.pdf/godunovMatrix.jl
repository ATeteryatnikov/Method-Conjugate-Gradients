...
function norma(x::Array{Float64,1})              
  result = 0e0                      # Переменной result присваивается                      
                                          # тип BigFloat.
   x::Array{Float64,1} = x.^2         # Все элементы вектора возводятся в квадрат.              
  result = sum(x)  # Все элементы вектора суммируются.
  return result::Float64   # функция возвращает переменную result.
end
...
####################################### Начало МСГ
x = fill(0.0,n)
local p::Array{Float64,1} = x
local s::Array{Float64,1} = x
local y::Float64 = 0.0
local q::Array{Float64,1} = x
local a::Float64 = 0.0
local y1::Float64 = 0.0
local b::Float64 = 0.0
...
while(y> norm_residual && k<limit)
  q::Array{Float64,1} = A*p
  a::Float64 = y/norma(q)
  x::Array{Float64,1} = x + a*p
  s::Array{Float64,1} = s - a*A'*q
  y1::Float64 = norma(s)
  b::Float64 = y1/y
  p::Array{Float64,1} = s + b*p
  k::Int=k+1
  y::Float64=y1
end
display(k)
