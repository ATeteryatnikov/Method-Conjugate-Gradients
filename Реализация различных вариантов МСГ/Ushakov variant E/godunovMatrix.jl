...
while(y> norm_residual && k<limit)
  Ed = A*d
  a = (r'*r)/((Ed)'*Ed)   # a - является матрицей из одного элемента.
...
max = 0e0
for i in 1:1:length(difference)
if (difference[i] > max)
  max = difference[i]
end
end
...
file = open(string(pathResult, "/maxDiffer.txt"), "w")
write(file, "maxDifference: ")
write(file, string(max))
close(file)
